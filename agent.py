...
def minimax_min_node(board, color, limit, caching = 0):
    ...
    values = []
    for c in children:
        values.append(minimax_max_node(c, color, limit-1)[1])
    
    min_value = min(values)
    move = list_of_moves[values.index(min_value)]

    if caching:
        cache[(board,color)] = (move, min_value)
...
def minimax_max_node(board, color, limit, caching = 0): #returns highest possible utility
    if caching:
        global cache
        if (board,color) in cache.keys():
            return cache[(board,color)]

    list_of_moves = get_possible_moves(board, color)
    if list_of_moves == [] or limit == 0:
        return (None, compute_utility(board,color))

    children = []
    for move in list_of_moves:
        children.append(play_move(board, color, move[0], move[1]))

    values = []
    for c in children:
        values.append(minimax_min_node(c, color, limit-1)[1])
    
    max_value = max(values)
    move = list_of_moves[values.index(max_value)]

    if caching:
        cache[(board,color)] = (move, max_value)
...
def alphabeta_max_node(board, color, alpha, beta, limit, caching = 0, ordering = 0):
    ...
    values = []
    for c in children:
        ut_val = max(ut_val, alphabeta_min_node(c,color, alpha, beta, limit-1)[1])
        values.append(ut_val)
